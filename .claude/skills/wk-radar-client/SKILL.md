...
#### Escrita (`POST /comercial/v1/pedido`) — OS-BACKEND-25

Contrato confirmado (schemas de request e response já compartilhados no histórico do projeto) — grava uma **lista** de pedidos por chamada. Pontos confirmados:
- **Request**: schema extenso (fiscal, transporte, faturamento, rateios, tributos por item) com só alguns campos plausivelmente obrigatórios pra um pedido simples de venda — `idFilial`, `idOperacaoComercial`, `idNaturezaOperacaoProduto`/`idNaturezaOperacao` (por item), `idClassificacao`, `idTabelaPreco` (por item), `idUnidadeVenda` (por item), `idCondicaoPagamento` (em `faturamento`). **Nenhum desses é sincronizado ou configurado em lugar nenhum do sistema hoje.**
- **Response de sucesso**: simples — `[{ "id": "string", "codigoIntegrador": "string" }]` (um por pedido enviado). `id` é o `idExternoErp` do pedido criado no Radar; `codigoIntegrador` ecoa o que foi enviado (ponte de ID externo, ver seção acima).
- **Response de erro de negócio** (ex: produto sem saldo suficiente no momento do envio) — formato **não confirmado**. `PedidoErpClientService` (ver `pedidos/pedido-erp-client.service.ts`) trata qualquer falha de forma genérica por enquanto.

**PENDÊNCIA BLOQUEANTE (confirmada com o usuário, não implementar sem isso)**: os valores fixos de `idFilial`/`idOperacaoComercial`/`idNaturezaOperacao`/`idClassificacao`/`idTabelaPreco`/`idUnidadeVenda`/`idCondicaoPagamento` que a empresa usa não foram fornecidos ainda — são valores fixos da empresa (confirmado que não variam por pedido), mas ainda não foram passados. `PedidoErpClientService.criar()` lança erro fail-closed imediatamente (nunca finge sucesso, nunca inventa esses IDs) até essa pendência ser resolvida — o resto do fluxo de criação de pedido (validação de escopo, cálculo por tipo de venda, regra de aprovação de desconto, persistência local) está pronto e testado, só a chamada real ao Radar fica bloqueada.
...
- **Valores fixos de referência para `POST /comercial/v1/pedido`** (OS-BACKEND-25) — `idFilial`, `idOperacaoComercial`, `idNaturezaOperacao` (por item), `idClassificacao`, `idTabelaPreco` (por item), `idUnidadeVenda` (por item), `idCondicaoPagamento`: confirmados como valores fixos da empresa (não variam por pedido), mas os valores em si ainda não foram fornecidos. `PedidoErpClientService.criar()` fica fail-closed até isso chegar — ver seção "Escrita" de Pedido acima.
- **Formato de erro de negócio de `POST /comercial/v1/pedido`** (ex: produto sem saldo suficiente) — não confirmado; `PedidoErpClientService` trata qualquer falha de forma genérica por enquanto.
