...
Choose the right approach based on scope: local ephemeral UI state vs. app/business state.

```dart
// Local ephemeral UI state (visual-only, not shared, not backed by API data):
// StatefulWidget is fine — e.g. animation progress, expanded/collapsed toggle.
...
// App/business state (shared across widgets, backed by API, or survives navigation):
// Project convention is Riverpod — never Provider/ChangeNotifier.
