...
// Project convention is Riverpod — theme state goes through a Notifier, not ChangeNotifier.
class ThemeState {
  final AppThemeType currentTheme;
  final ThemeMode themeMode;

  const ThemeState({this.currentTheme = AppThemeType.light, this.themeMode = ThemeMode.system});

  ThemeState copyWith({AppThemeType? currentTheme, ThemeMode? themeMode}) {
    return ThemeState(
  ...
  ThemeData get darkTheme => _getThemeData(currentTheme, Brightness.dark);
  ...
  ThemeData _buildDarkTheme(Brightness brightness) => _buildLightTheme(brightness);
...
class ThemeNotifier extends Notifier<ThemeState> {
  ...
  void setTheme(AppThemeType theme) => state = state.copyWith(currentTheme: theme);
...
class ThemedApp extends ConsumerWidget {
  ...
  const ThemedApp({required this.builder});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final themeState = ref.watch(themeProvider);
    return builder(context, themeState);
  }
}
