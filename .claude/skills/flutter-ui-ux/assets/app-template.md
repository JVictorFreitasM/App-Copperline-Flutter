...
Riverpod-based state management (project convention — never Provider/ChangeNotifier for app/business state).
...
class AppState {
  ...
  const AppState({this.isLoading = false, this.errorMessage});
...
class AppStateNotifier extends Notifier<AppState> {
  ...
  void clearError() => state = AppState(isLoading: state.isLoading, errorMessage: null);
}
...
// AsyncNotifier fits this case: state depends on an async operation (login call).
class UserNotifier extends AsyncNotifier<User?> {
  ...
  Future<void> login(String email, String password) async {
    ...
    state = await AsyncValue.guard(() async {
      // Login logic here (call the API service)
...
// Local, ephemeral, purely-visual state (not shared, not API-backed) can still
// use StatefulWidget directly — this does not need to go through Riverpod.
import 'package:flutter/material.dart';
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          CustomButton(
            text: 'Login',
            onPressed: () {
              ref.read(userProvider.notifier).login('user@example.com', 'senha');
