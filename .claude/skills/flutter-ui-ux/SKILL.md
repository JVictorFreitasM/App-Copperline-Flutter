...
name: flutter-ui-ux
description: |
  Comprehensive Flutter UI/UX development skill for creating beautiful, responsive, and animated mobile applications, aligned with the project's Riverpod state management convention. Use when user asks to:
...
## Project State Management Convention

This project uses **Riverpod** for app/business state — never Provider/ChangeNotifier (see the `flutter-widget` and `nestjs` skills for the full project convention rationale). When following the patterns below:

- **Local, ephemeral, purely-visual state** (animation progress, a toggle's expanded/collapsed flag, drag offset, scroll position) is fine to keep in a `StatefulWidget` — this is standard even in Riverpod codebases and does not need a provider.
- **App/business state** (anything backed by an API call, shared across screens, or that needs to survive navigation — theme selection, logged-in user, cached data) always goes through a Riverpod `Notifier`/`AsyncNotifier`, exposed via `ConsumerWidget`/`ref.watch`.
- Any example in the reference files below (`widget-patterns.md`, `theme-templates.md`, `assets/app-template.md`) that involves app/business state uses Riverpod. If you see `ChangeNotifier`/`Provider` package examples anywhere else, translate them to the Riverpod equivalent before using — don't introduce the `provider` package into this project.

## Project Visual Identity

The color values, typography scale, spacing, and component patterns (buttons, cards, list items, progress bars) shown as generic examples in the reference files below are illustrative only. This project has its own concrete design tokens — see skill `design-system`. Apply those tokens (colors, border radius, component shapes) on top of the structural/animation patterns documented here, rather than the placeholder colors used in the examples.
